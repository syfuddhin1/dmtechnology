...
export async function addBranchInfoByDate(data) {
  ...
  try {
    // update day data date
    // Check if branch data already exists for the given date and code
    let existingBranchData = await branchModel.findOne({
      code: data.code,
      date: data.date
    });
    ...
    if (existingBranchData) {
      ...
      branchData = await branchModel.findOneAndUpdate(data, { new: true });
    } else {
      // If data doesn't exist, create new entry
      branchData = await branchModel.create(data);
    }
    ...
    // 5 = Friday, 6 = Saturday
    if (oneDayAhead.getDay() === 4 || oneDayAhead.getDay() === 5) {
...
export async function getVoucherData() {
  ...
  try {
    ...
    // console.log("addVoucher", data);
    const voucherData = await voucherModel
      .find({ branch: name })
      .lean()
      .sort({ date: -1 })
      .limit(25);
...
export async function getLedgerData({ dateFrom, dateTo, accountName }) {
  await connectMongo();
  try {
    const session = await auth();
    const name = session?.user?.name;
    ...
    // console.log(query);
    const voucherData = await voucherModel.find(query).lean().sort({ date: 1 });
    // Calculate previous balance up to the start date
    ...
    const prevTransactions = await voucherModel.find(prevBalanceQuery).lean();

    let prevBalance = 0;
    let prevCreditTotal = 0;
    let prevDebitTotal = 0;
    for (const transaction of prevTransactions) {
      if (transaction.creditAccounts === accountName) {
        prevBalance -= transaction.amount;
        prevCreditTotal += transaction.amount;
      } else if (transaction.debitAccounts === accountName) {
        prevBalance += transaction.amount;
        prevDebitTotal += transaction.amount;
      }
    }

    // Add prevBalance to the return object
    const openingBalance = ({
      date: startDate,
      voucherCode: 'Opening Balance',
      balanceType: prevBalance >= 0 ? 'Dr' : 'Cr',
      balance: prevBalance,
      prevCreditTotal,
      prevDebitTotal
    });
